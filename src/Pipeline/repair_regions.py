...
def repair_regions(df, label, max_size = None, lookbackSize = None, extra_features = None):
  ...
  error_cols = [i for i in df_att.columns if 'delay_error' in i or i == (label+'_error')]
  i = 0
  ...
  drop_cols = [i for i in df_att.columns if 'delay_error' in i] + [label] + [label+'_error']
...
for i in range(len(df_cols)):
  ...
  n_Series, n_error = interpolation(Series, error, **config[label.split('_')[0]] )
