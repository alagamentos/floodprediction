...
def derivative_threshold(timeSeries, threshold, plot = True,
    ...
    if plot:
        ...
        ax2.axhline(-threshold, c = C2, lw = lw) 
        ax2.plot(timeSeries.index[plt_start:plt_stop],d_ts[plt_start:plt_stop], c = C1, lw = lw)
...
def derivative_zero(timeSeries, n_zeros, non_zero = False,
    ...
    if plot:
        ...
        for d in const:
            ...
            i = i + 1
        ax1.plot(timeSeries.index[plt_start:plt_stop],timeSeries[plt_start:plt_stop], lw = lw, c = C1)
        if ylim:
            ax1.set_ylim(ylim[0],ylim[1])
        ax2 = plt.subplot(211)
        ax2.plot(timeSeries.index[plt_start:plt_stop],d_ts[plt_start:plt_stop], lw = lw, c = C1 )
